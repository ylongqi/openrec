...
class UserPMF(PMF):
    ...
    def _input_mappings(self, batch_data, train):
        ...
        default_input_map = super(UserPMF, self)._input_mappings(batch_data=batch_data, train=train)
        if train:
    ...
    def _build_user_extractions(self, train=True):
        ...
        if train:
            ...
            self._user_f = MultiLayerFC(in_tensor=self._user_feature_input, dims=self._dims, l2_reg=self._l2_reg_mlp,
                            dropout_mid=self._dropout_rate, scope='user_MLP', reuse=False)
        else:
            self._user_f_serving = MultiLayerFC(in_tensor=self._user_feature_serving, dims=self._dims, l2_reg=self._l2_reg_mlp,
