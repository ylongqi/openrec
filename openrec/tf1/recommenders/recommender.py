...
class _RecommenderGraph(object):
    ...
    def register_operation(self, operation, identifier='default'):
        ...
        tf.add_to_collection('openrec.tf1.recommender.operations.'+identifier, operation)

    def register_loss(self, loss, identifier='default'):

        self._loss_identifier_set.add(identifier)
        tf.add_to_collection('openrec.tf1.recommender.losses.'+identifier, loss)

    def register_output(self, output, identifier='default'):
        
        self._output_identifier_set.add(identifier)
        tf.add_to_collection('openrec.tf1.recommender.outputs.'+identifier, output)
    ...
    def get_operations(self, identifier='default'):
        ...
        with self._tf_graph.as_default():
            return tf.get_collection('openrec.tf1.recommender.operations.'+identifier)

    def get_losses(self, identifier='default'):
        
        with self._tf_graph.as_default():
            return tf.get_collection('openrec.tf1.recommender.losses.'+identifier)

    def get_outputs(self, identifier='default'):

        with self._tf_graph.as_default():
            return tf.get_collection('openrec.tf1.recommender.outputs.'+identifier)
