...
from openrec.utils.samplers import DRRSampler, DRREvaluationSampler
import numpy as np
...
#item_feature = np.load('dataset/lastfm/item_feature.npy')

total_users = 992
total_items = 14598 

user_dict = {'gender': 3, 'geo': 67}
item_dict = {'geo': 407, 'type': 7, 'tag': 1092, 'id': total_items}
...
train_sampler = DRRSampler(user_feature=user_feature, batch_size=batch_size, max_seq_len=max_seq_len, dataset=train_dataset, num_process=1)
test_sampler = DRREvaluationSampler(user_feature=user_feature, dataset=test_dataset, max_seq_len=max_seq_len)
