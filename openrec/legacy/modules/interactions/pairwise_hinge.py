...
class PairwiseHinge(Interaction):

    """
    The PairwiseHinge module minimizes the pairwise hinge loss as follows (regularization and bias terms \
    are not included):
    
    .. math::
        \min \sum_{(i, p, n)} [m - u_i^T v_p + u_i^T v_n]_+
    ...
    *positive item* :math:`p` and *negative item* :math:`n`, respectively.
    ...
    def __init__(self, user, item=None, item_bias=None, p_item=None, p_item_bias=None, 
                n_item=None, n_item_bias=None, margin=1.0, train=None, scope=None, reuse=False):
        ...
        super(PairwiseHinge, self).__init__(train=train, scope=scope, reuse=reuse)
    ...
    def _build_training_graph(self):
        ...
        with tf.variable_scope(self._scope, reuse=self._reuse):
            ...
            dot_user_neg = tf.reduce_sum(tf.multiply(self._user, self._n_item),
                                         ...
                                         keep_dims=True,
                                         name="dot_user_neg")
            self._loss = - tf.reduce_sum(tf.maximum(self._margin - (dot_user_pos + self._p_item_bias) +
                                                        (dot_user_neg + self._n_item_bias), 0))
